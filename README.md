...
- **条目标签**改成文件基名。文件比卡内的 `name` 字段新 —— 作者往往先改贴子和文件名，很久之后才想起改卡里的名字
- **版本关键词**跟着新名走，否则搜索还在找已经不存在的旧名字，改名提示也不会消失
- **MVU 版文件名**：它的名字是「卡名 + MVU 标记」，标记保留、前面的卡名换掉，`创世回廊5.2 MVU版本.json` 变成 `龙娘回廊！5.3 MVU版本.json`。名字里没有 MVU 标记的副本不动，因为分辨不出哪一段是卡名
- **卡片头像跟着搬**。Tavern 是按文件名给卡片配头像的（`originals/cards/<卡名>.png`），只改卡不管图的话，卡会退回默认头像 —— 这是看得见的损失，比标签写错更闹心。原版和 MVU 版各自的图都会跟着走，目标位置已经有图时不覆盖
