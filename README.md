...
- 黄色「原卡正在调试中」表示那个对话最近几分钟还在写东西，也就是调试还没结束。没有这一档的话，刚点完「前去调试」就会立刻显示成已经调完了
- 黄色「原卡调试有改动，建议查看」表示调试对话把改动写回了这张卡（日志里的 `card.save`），那些改动值得读一遍。磁盘上没有「谁读过这个对话」的记录，所以这句陈述的是**卡被改过**这个事实，不是「你没看过」这个猜测
