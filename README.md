...
1. **有更新的**
2. **贴子改过名的**
3. **工作台没看过的**（也包含调试之后文件又被改动的）
4. 其余

同一档内保持配置里的顺序。在这个下拉里选了别的排序，就完全按选的来，不再叠加默认这几档，因为挑好的顺序才是正在找的顺序。
...
**导入新版**会先解析确认那确实是一张人物卡，通过后才备份并覆盖原版卡，选错文件不会改坏现有卡。

**导入会跟随新卡的名字。** 作者改名是常事（`创世回廊` 变成了 `龙娘回廊`），下载来的新卡文件自带的也是新名字。只写内容而保留旧文件名的话，文件夹里就躺着一份名不符实的卡，条目标签也和卡里写的不一致。所以导入手时会：

- **原版卡**改用导入文件的文件名。如果新卡本来就是直接下载进卡片目录的（很常见），那已经存在的那个新文件会被采用，旧文件删除，删之前照例先备份
- **条目标签**跟着卡内的 `name` 字段走
- **MVU 版**跟着改：它的名字是「卡名 + MVU 标记」，标记保留、前面的卡名换成新的。`创世回廊5.2 MVU版本.json` 会变成 `龙娘回廊！5.3 MVU版本.json`。名字里没有 MVU 标记的副本不动，因为分辨不出哪一段是卡名
- 万一新名字已经被卡片目录里**另一个文件**占用，就保持旧名只更新内容，不覆盖别人的文件

导入完成的提示里会写出这次的改名，形如 `创世回廊！5.2_1.json → 龙娘回廊！5.3.json`。

**卡图不归插件管。** 卡片头像由 Tavern 自己按卡片文件解析并缓存，改完名刷新一下页面（或重开 Tavern）就会重新读。插件面板里的头像是按原版卡路径现取的，改名后立刻跟着变。
