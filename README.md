...
**刷新令牌的办法**：`auth_token` 只在页面加载时从网址的 `#token=` 片段里被写入一次，站点的登录回调不碰它，所以单纯刷新页面并不一定会更新。可靠的判断办法是先清掉旧的，再完整走一次登录：

```js
// 1. 在索引站控制台清掉旧值，这样才能看出是否写入了新的
localStorage.removeItem('auth_token')
```

2. 退出登录，再用 Discord 重新登录
3. 回到站点执行下面这句，看有没有值：
...
拿到新值就粘回面板。如果返回 `null`，说明登录流程本身不发这个令牌，它由站外签发，需要从社区侧获取。
...
**提示令牌已过期**：索引站的令牌有效期约 7 天，过期后检索贴子会返回 401。刷新方式见上面「索引站授权」一节：先 `localStorage.removeItem("auth_token")` 清掉旧值，再退出登录重新登录一次，然后重新取。注意浏览器里看着还登录着是正常的，登录态靠 cookie，令牌是另一条凭据，两者不会互相刷新。
