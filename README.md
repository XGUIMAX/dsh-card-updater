...
**版本是怎么比的**。基线是**卡片自己的 `character_version`**，因为它说的是这台机器手上实际有什么；贴子上的数字会随作者改个标题就漂移。贴子那一侧的版本有三个来源，优先级从高到低：

| 来源 | 说明 |
| --- | --- |
| **索引站记录的版本** | 站点为这个贴子维护的字段。有值就直接用它，不做任何猜测 |
| **贴子里的版本标注** | 「版本 5.3」「5.3版更新」「version 1.2」「beta0.5.5」这类明确写法 |
| **标题里的日期** | 「9.21更新」这类。这是兜底，因为日期几乎总比版本号大 |

前两级都空的时候才落到日期，比的是「比卡内版本大」而不是「比上次大」，所以作者只改日期不更新卡时，报的是有新版，属于宁可多报。
