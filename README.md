...
**提示「页面上没有找到「某某」，无法定位这张卡」**：说明当前链接里没有这个词。两种常见原因。一是「版本关键词」把卡名里的标点也带上了，比如卡叫 `创世回廊！5.2_1` 而索引站上写的是 `「创世回廊」`，搜索词里那个感叹号会让结果直接变成零条；把「版本关键词」改成实际出现在贴子标题里的写法，或者整个清空，让它改从卡片自己的名字提取（1.18.1 起尾部标点会被自动去掉）。二是这个链接本来就匹配的是另一张卡，清空「主要链接」再检测一次就会重新匹配。
