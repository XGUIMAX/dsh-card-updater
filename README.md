...
| **Discord 更新消息正文** | 索引站指向的那条更新消息（需填 Discord 令牌） | 从作者原话里取版本，例如「9月21日更新5.3」 |
...
**贴子改名会单独提示。** 作者把「创世回廊」改成「龙娘回廊」这类事会发生，贴子还是那张贴子、卡还是那张卡，但按旧名字去找的人会以为它没了。插件会记下上次读到的标题，下一次检测如果标题变了，就在来源检索那一行写出「贴子已改名：旧名 → 新名」。

**Discord 授权是可选的，但能补上最大的一个缺口。** 社区索引站只收录贴子的标题和摘要，而作者把版本号写在正文里（「9月21日更新5.3」），索引站看不到。填一个 Discord 令牌之后，插件会去读索引站**已经指明的那一条**更新消息，从作者的原话里取版本号。一次请求，一条消息，不遍历频道。不填也完全可用，只是版本号只能靠标题推断。获取方式和注意事项写在「合并设置 → Discord 访问授权」里。

卡片上这四个动作，区别只在「原版卡的新内容从哪来」和「要不要顺手合并」：

| 动作 | 做什么 | 需要联网 |
| --- | --- | --- |
| **导入新版** | 把你**已经下载好**的卡文件复制成原版卡 | 不 |
| **更新原版** | 从卡片的来源链接**下载**新卡来覆盖原版卡 | 是 |
| **合并到 MVU** | 把**当前原版卡**的内容同步进 MVU 版 | 不 |
| **更新并合并** | 上两步连做：先更新原版卡，再合并进 MVU | 是 |

于是日常有两条路径：

```
手动：检测 → 有新版 → 自己下载 → 导入新版 → 合并到 MVU
自动：检测 → 有新版 → 更新并合并
```
...
**更新原版** 和 **更新并合并** 需要卡片配好了能直接下载的来源。来源挂着密码、权限或赞助时下载会失败，那种情况走上面手动那条路。所有写入动作都会先备份。
