...
合并结果与磁盘上已有的内容完全一致时不会写卡，也不会留下快照 —— 连点两次「合并」不会重写一遍整张卡，也不会用重复快照挤掉更早的回滚点。
...
**导入会跟随新卡的名字。** 作者改名是常事（`创世回廊` 变成了 `龙娘回廊`），下载来的新卡文件自带的也是新名字。只写内容而保留旧名字的话，面板、索引检索和文件夹三者会各说各的。所以导入时会一次同步四处：

- **原版卡文件名**改用导入文件的文件名。如果新卡本来就是直接下载进卡片目录的（很常见），那已经存在的那个新文件会被采用，旧文件删除，删之前照例先备份
- **条目标签**用卡内 `name` 字段。文件名是你可以随便改的，卡内的名字是作者写进去的、也是这张卡对外的自称；两者冲突时以卡内为准，文件名只在卡里没写名字时兜底
- **版本关键词**跟着文件基名走，因为它要匹配的是贴子标题，而标题跟着作者的新名字变。卡内 `name` 不参与这一项
- **MVU 版文件名**：它的名字是「卡名 + MVU 标记」，标记保留、前面的卡名换掉，`创世回廊5.2 MVU版本.json` 变成 `龙娘回廊！5.3 MVU版本.json`。名字里没有 MVU 标记的副本不动，因为分辨不出哪一段是卡名
- **MVU 副本自己的名字也带上标记**。卡列表是按卡内的名字显示的，副本的内部名和原版一模一样时，列表里会出现两个看不出区别的条目（都叫 `创世回廊5.3`），只能靠下面那行文件名分辨。副本是你自己派生的卡而不是作者的原文，给它加标记不算改别人的东西；写回时保留 `kind`、`version`、`meta` 整套外壳
- **卡片头像跟着搬**。Tavern 是按文件名给卡片配头像的（`originals/cards/<卡名>.png`），只改卡不管图的话，卡会退回默认头像 —— 这是看得见的损失，比标签写错更闹心。原版和 MVU 版各自的图都会跟着走；**MVU 版直接用原版那张新图**，因为它们是同一张卡，沿用旧 MVU 图会显示上一版的美术，看起来像头像没更新
- **已有的对话跟着指向新文件**。Tavern 把每个对话用的卡记成路径（`cards/<卡名>.json`），卡改名后这些路径就悬空了，对话会报「人物卡已删除」，而卡就在旁边、聊天记录一条没少。改名时会把这些指针一并改到新路径，索引文件在写之前先备份
- 万一新名字已经被卡片目录里**另一个文件**占用，就保持旧名只更新内容，不覆盖别人的文件

**已经导入过的卡也能补同步**：再点一次「导入新版」选当前这张原版卡。插件会识别出选的就是它自己，不复制内容，只把上面那几处名字对齐。这是不重新导入也能触发同步的唯一入口。

导入完成的提示会写出这次动了什么，形如 `创世回廊！5.2_1.json → 龙娘回廊！5.3.json · 关键词改为 龙娘回廊 · MVU 版改名 龙娘回廊！5.3 MVU版本.json`。

**卡图不归插件管。** 卡片头像由 Tavern 自己按卡片文件解析并缓存，改完名刷新页面（或重开 Tavern）会重新读；新头像要放在卡片目录里、与卡片文件同名。插件面板里的头像是按原版卡路径现取的，改名后立刻跟着变。
...
**卡被删了但有残留**：在 Tavern 里删掉一张卡只会删文件，配置里指向它的条目会留下，面板上就多出一条点「检测」怎么都没反应的记录。这种条目的状态标签现在会直接写「卡文件已不在」，并排到列表最后；工具栏的「清理失效条目」会带上数量，点它一次把这类条目清掉（先弹确认，并列出是哪几个）。

不会自动清，也不会碰只缺一半的条目：外置盘没插、卡临时挪走，看起来和「被删了」一模一样，而这两种情况想要的结果正好相反。所以判断只在每次读取状态时现算，文件回来了标记就消失。清理只删配置条目，不动任何卡片文件，但那条条目的发布链接会一起丢掉。
...
备份保留策略：每张卡文件保留最近 **5** 份，每个标签（导入 / 合并 / 手动 / 还原前）另各留最新 1 份，总量上限 **1000** 份。按每张卡文件计算，所以加卡不会挤掉别的卡的历史；总量上限只防失控增长，1000 份完整快照大致是几个 GB 的量级。
...
面板浏览器本身能跨盘：顶部的**路径输入框**接受 `D:\备份`、`D:`、带引号的粘贴路径、正斜杠，以及相对卡片目录的名字，回车或点「转到」即可；「盘符」按钮列出本机所有盘（`C:` `D:` …），盘根再点「上一层」就回到这个盘符表 —— 放在别的盘上的文件夹因此也够得着。换完目录旧目录里的快照会自动搬到新目录，规则是**先全部复制、成功之后才删原目录**：中途出错就不改配置、原目录原封不动，修好再点一次即可（已经复制过去的不重复复制）。新旧目录里同名、内容也相同的算同一份，只留一份；同名但内容不同的两边都留着，并在面板上告诉你有几个、在哪个目录。想改回来点「改回默认目录」。备份目录不能选在卡片目录里面，否则备份文件会被当成人物卡。

拼写不同但其实是同一个文件夹（大小写、末尾分隔符）不会被当成换目录，也不会白白重搬一遍。

自动清理只认本插件自己写的快照文件名（`<毫秒>__<标签>__<卡文件名>.json`）。所以就算把备份目录指到一个放满别的 JSON 的文件夹，清理也不会碰它们；搬目录时同样只搬自己的快照，删除和还原单个快照也按同一条规定校验 —— 面板上删不掉不属于本工具的 JSON。
...
**「检测更新」是怎么工作的**：面板右上角那个按钮显示的是插件自身在 GitHub 上的版本状况。发现新版时**点它就直接更新**，不用去浏览器：插件跑在 git 检出里，更新就是一次 `git pull --ff-only`。用 `--ff-only` 是因为仓库里有未提交的改动时应该停下来告诉你，而不是自己合成一个没人要的合并提交。拉取前后会各读一次修订号，所以成功提示里写的是 `旧 → 新`，而不是笼统的一句"已更新"。更新完**重启 DSH** 生效。

**报「这个插件不是 git 检出，没法自动更新」**：说明安装方式不是从仓库 checkout，而是从 npm 或压缩包来的。那种情况用 `dsh plugin --profile <profile> add` 重新安装，之后就能自更新了。
...
```powershell
npm test              # 下面三套一起跑
node test/run.mjs     # 宿主端：路径、保留策略、配置、配对、合并、备份、导入改名
node test/client.mjs  # 浏览器端：字典、样式表、面板转换函数
node test/real.mjs    # 用本机真实卡片跑合并，验证 MVU 结构不被破坏
node test/perf.mjs    # 只是计时，不是断言
```

三套都跑在临时目录里。宿主端通过把 `DSH_HOME` 指到一个临时沙箱来隔离 —— 插件所有路径都是从它推出来的，所以沙箱里的运行读不到也写不到真实卡片目录。`test/real.mjs` 会把本机最小的那对卡**复制**进沙箱再合并，逐对检查扩展、世界观书和开场白里的状态栏占位符没有丢失，并且连合两次第二次不再写盘；找不到卡片时自动跳过。

`test/harness.mjs` 把两个半边的源码**在内存里**改写成可注入的模块 —— 宿主端去掉 `node:` 导入，浏览器端喂一个假的 `window.__ModuleLoader__` 和 `react` —— 源码文件本身不动，所以这些内部函数不必为了测试而导出。
