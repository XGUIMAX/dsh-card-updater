...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      'flag.bookPrefer': '作者改过的世界书条目跟着更新',
      'flag.bookPrefer.d':
      ...
      'merge.bookHeld': '保留了 MVU 版自己的 {n} 条变量规则，没有换成作者版（两套变量协议不通用，换过去结算会失效）；把作者那份里的内容规则并进来属于细节适配，留到单卡调试时做',
    ...
    const en = {
      ...
      'flag.bookPrefer': "Follow the author's revision of a world book entry",
      'flag.bookPrefer.d':
      ...
      'merge.bookHeld': "Kept the MVU copy's own {n} variable-rule entries (the author's version uses another protocol; swapping it would break settlement); folding the author's content rules in is detail work for a single-card debug pass",
    ...
    const CSS = [
      ...
      //
      // The wide-state box matches the shipped rows' math — `calc(100% + 4px)`
      // with a `margin` of `4px -2px` — so this row is exactly as wide as the
      // market row above it and the gear below it, on the same axis.
      //
      // Tried once without the nudge, filling the seat instead. That made this
      // row disagree with the two neighbours it has to line up with, so it is
      // back to matching them. Whatever offset the desktop shell shows is shared
      // by all three rows, which is a container problem, not this row's.
    ...
    function SettingsTab({ cfg, setCfg }) {
      ...
      return h(
        ...
        h(
          ...
          flag('autoBumpVersion', t('flag.autoBump'), t('flag.autoBump.d'), false),
        ),
        h(
          'div',
          { className: 'dcu-card flat' },
