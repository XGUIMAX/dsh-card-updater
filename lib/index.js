...
async function importPlain(cardId, fromPath) {
  ...
  // The MVU copy follows too. Its name is chosen by hand, but it is still a card
  // name followed by an MVU marker — 创世回廊5.2 MVU版本 — and the card-name half
  // is exactly the part that goes stale. So the marker is kept and the front is
  // replaced. A copy with no MVU marker in its name is left alone: there is
  // nothing there to tell a card name apart from the rest of it.
  const mvuPath = entry.mvu?.path
  const mvuMark = mvuPath
    ? path.basename(mvuPath).replace(/\.json$/i, '').match(/^(.*?)[\s_-]+(MVU.*)$/i)
    : null
  if (renamed.from && mvuPath && mvuMark) {
    const nextStem = `${renamed.to.replace(/\.json$/i, '')} ${mvuMark[2]}`
    const nextMvu = path.join(path.dirname(mvuPath), `${nextStem}.json`)
    if (path.resolve(nextMvu) === path.resolve(mvuPath)) {
      // Already says the right thing; nothing to do.
    } else if (exists(nextMvu)) {
      log('import: mvu copy kept its name, the new one already exists')
    } else {
      try {
        fs.renameSync(mvuPath, nextMvu)
        entry.mvu.path = nextMvu
      } catch (e) {
        log('import: mvu rename failed:', e && e.message ? e.message : e)
      }
    }
  }

  // The label is the handle the panel shows, so it follows the card's own name.
  if (cardName && cardName !== entry.label) entry.label = cardName
