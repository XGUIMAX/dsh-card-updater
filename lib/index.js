...
/**
 * A script that draws or hides the old status bar. Converting a card replaces
 * that mechanism wholesale, since the copy runs a single variable-driven view,
 * so putting the author's version back means two scripts watching one
 * placeholder and script order deciding which of them is seen.
 */
function looksLikeStatusBar(rule) {
  const text = [rule.scriptName, rule.findRegex, rule.replaceString].map((v) => String(v ?? '')).join('\n')
  return /StatusPlaceHolder|<mvu[\s/>-]|mvu-status|状态栏/i.test(text)
}
 ...
 * @param {boolean} preferOriginal - take the author's text for an entry that
 *   exists on both sides with different content. The copy's text in that case is
 *   usually the older one, since the copy is a conversion of an earlier release,
 *   so leaving it alone is what kept an author's revisions from arriving. Turn it
 *   off to protect local edits instead.
 * @returns {{book: object, res: {added: number, updated: number, refreshed: number, conflicts: string[]}}}
 */
function mergeBook(mvuBook, plainBook, preferOriginal) {
  const res = { added: 0, updated: 0, refreshed: 0, conflicts: [] }
...
function mergeCards(mvuCard, plainCard, strategy, opts) {
  ...
  if (strategy !== 'minimal' && plainData.character_book) {
    const merged = mergeBook(mvuData.character_book, plainData.character_book, settings.preferOriginal !== false)
...
async function checkPrimary(entry, feedItems) {
  ...
  try {
    ...
    let keyword = String(state.match || '').trim()
    if (!keyword) {
      const derived = searchTermOf(entry)
    ...
    const scoped = scopeText(text, keyword)
    const scope = scoped || (thread ? text : '')
    if (!scope) {
      throw new Error(
        keyword
          ? `页面上没有找到「${keyword}」，无法定位这张卡`
...
function syncNames(entry, renamed) {
  const out = { label: null, keyword: null, mvu: null, mvuName: null, nameTouched: [], avatar: false, chats: 0 }
        ...
        try {
          ...
          if (exists(fresh) && !exists(target)) {
            try {
              ...
              moveCardImages(wasStem, nowStem)
            }
          } else {
            moveCardImages(wasStem, nowStem)
          }
  ...
  if (mvuPath && marker) {
    try {
      ...
      const target = inner && !/MVU/i.test(inner) ? `${inner} ${marker}` : inner
      if (target) {
        // One card, one name.
        //
        // A card states its name in four places: `name` at the top level, `name`
        // again under `data`, the world book it ships, and `extensions.world`,
        // which names that book. A conversion writes some of them and leaves
        // others, so they drift. On this machine one copy read 5.2 in two of them
        // and 5.3 in a third, another pointed `extensions.world` at a book name
        // that no longer existed — a key with no matching lock — and a third had
        // a stray "- " in front of both. They are set to one value because two of
        // them are a key and the thing it opens.
        const touched = []
        const top = copy.payload
        const data = top.data && typeof top.data === 'object' ? top.data : null
        const book = (data && data.character_book) || top.character_book
        const ext = data && data.extensions && typeof data.extensions === 'object' ? data.extensions : null
        if (top.name !== target) {
          top.name = target
          touched.push('name')
        }
        if (data && data.name !== target) {
          data.name = target
          touched.push('data.name')
        }
        if (book && book.name !== target) {
          book.name = target
          touched.push('书')
        }
        if (ext && ext.world !== target) {
          ext.world = target
          touched.push('world')
        }
        if (touched.length) {
          backupFile(entry.mvu.path, 'before-name-unify')
          writeText(entry.mvu.path, buildCardText(copy.shell, copy.outer, copy.payload))
          out.mvuName = target
          out.nameTouched = touched
        }
      }
    } catch (e) {
      log('sync: mvu name unify failed:', e && e.message ? e.message : e)
    }
  }
  return out
}
...
async function importPlain(cardId, fromPath) {
  ...
  // The original's own avatar moves with it. Tavern pairs the two by file name, so
  // a renamed card with an unmoved picture shows the default avatar.
  if (renamed.to) {
    synced.avatar = moveCardImages(renamed.from.replace(/\.json$/i, ''), renamed.to.replace(/\.json$/i, ''))
...
/**
 * Bring every copy's four names into line with each other.
 *
 * A card states its name in the top level `name`, again under `data`, in the
 * world book it ships, and in `extensions.world`, which names that book. A
 * conversion writes some of them and leaves others, so they drift; on this
 * machine seven of eight copies had at least two of the four disagreeing, one
 * pointed `extensions.world` at a book name that no longer existed, and one read
 * 5.2 in two places and 5.3 in a third. This is the pass that settles them, and
 * it is safe to run again: a card already in agreement is only read.
 * @returns {Promise<{items: object[], changed: number}>} one row per card.
 */
async function runUnifyNames() {
  if (!db.loaded) loadConfig()
  const items = []
  for (const entry of db.cfg.cards || []) {
    if (!entry.mvu || !entry.mvu.path) {
      items.push({ id: entry.id, label: entry.label, ok: true, skipped: '没有 MVU 版' })
      continue
    }
    try {
      const res = syncNames(entry, null)
      items.push({
        id: entry.id,
        label: entry.label,
        ok: true,
        name: res.mvuName,
        touched: res.nameTouched || [],
        renamed: res.mvu,
      })
    } catch (e) {
      items.push({
  ...
  saveConfig()
  return { items, changed: items.filter((i) => i.touched && i.touched.length).length }
}
...
function cardWorkspaceState() {
  ...
  try {
    ...
    const index = JSON.parse(readText(path.join(TAVERN_DATA, 'index.json')))
...
async function handleAction(body) {
  ...
  switch (action) {
    ...
    case 'unifyNames':
      return { ok: true, result: await runUnifyNames() }
    case 'save':
