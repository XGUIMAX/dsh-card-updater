...
function backupFile(p, tag) {
  ...
  ensureDir(backupDir())
  const dst = path.join(backupDir(), `${Date.now()}__${tag}__${baseOf(p)}`)
  copyFile(p, dst)
  return dst
}
...
function syncNames(entry, renamed) {
  const out = { label: null, keyword: null, mvu: null, mvuName: null, avatar: false, chats: 0 }
...
/**
 * Tags this plugin writes into a snapshot name.
 *
 * A folder that is not the default one may hold files of its own, so every
 * read, every prune and every move is limited to names carrying one of these
 * tags. Pointing the setting at a folder full of unrelated JSON must not put
 * that JSON in reach of the retention pass, which deletes.
 */
...
function runSetBackupDir(raw, move) {
  ...
  if (next !== previous) {
    ...
    const probe = path.join(next, `.dsh-card-updater-write-test-${Date.now()}`)
    try {
  ...
  if (move && next !== previous) {
    ensureDir(next)
    for (const name of listNames(previous, (n) => isBackupName(n))) {
      ...
      try {
        copyFile(path.join(previous, name), dst)
        fs.unlinkSync(path.join(previous, name))
        moved += 1
      } catch (e) {
  ...
  db.cfg.backupDir = next === DEFAULT_BACKUP_DIR ? '' : next
  saveConfig()
  return { dir: next, previous, moved, kept, isDefault: next === DEFAULT_BACKUP_DIR }
}
...
async function handleAction(body) {
  ...
  switch (action) {
    ...
    case 'backupList': {
      let all = []
      try {
        all = fs.readdirSync(backupDir()).filter((n) => isBackupName(n))
      } catch {
        all = []
      }
      const items = all
