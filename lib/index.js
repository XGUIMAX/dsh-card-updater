...
function syncNames(entry, renamed) {
  const out = { label: null, keyword: null, mvu: null }
  ...
  // The MVU copy carries the card name and an MVU marker; the marker stays and the
  // front follows. A name with no marker is left alone, since there is nothing
  // there to tell a card name apart from the rest of it.
  ...
  const mark = mvuPath
    ...
    : null
  if (mvuPath && mark) {
    const nextMvu = path.join(path.dirname(mvuPath), `${stem} ${mark[2]}.json`)
    if (path.resolve(nextMvu) !== path.resolve(mvuPath)) {
      if (exists(nextMvu)) {
        ...
        try {
          ...
          // the old MVU file's own picture across would leave the previous
          // release's art sitting under the new name, which reads as an avatar
          // that did not update. The freshly imported card's picture is reused,
          // and only when there is none does the old one move instead.
          ...
          out.chats = (out.chats || 0) + repointChats(`cards/${wasStem}.json`, `cards/${nowStem}.json`)
        } catch (e) {
