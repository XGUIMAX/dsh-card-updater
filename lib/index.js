...
const DEFAULT_CFG = {
  ...
  indexToken: '',
  /**
   * Optional, and only ever used to read one message. The index publishes a
   * thread's metadata but not its text, and authors write the version where the
   * metadata cannot see it — "9月21日更新5.3" sits in the post body. With a
   * Discord token the plugin can fetch exactly the message the index points at
   * and read the version out of what the author actually wrote.
   */
  discordToken: '',
  cards: [],
}
...
function normalizeCfg(raw) {
  ...
  if (raw.discordToken !== undefined) cfg.discordToken = String(raw.discordToken || '')
...
function threadSummary(payload) {
  const wanted =
    /^(title|name|thread_name|thread_title|last_message|last_message_at|updated_at|update_time|created_at|message_count|reply_count|first_message_excerpt|description|source_message_at)$/i
  const lines = []
  const seen = new Set()
  let title = ''
  let version = null
  ...
  const walk = (node, depth, insideUpdate) => {
    ...
    for (const [key, value] of Object.entries(node)) {
      ...
      // Where that message lives, so the plugin can read the one post the index
      // points at rather than crawling the channel.
      if (key === 'message_link' && inUpdate && typeof value === 'string' && !messageLink) {
        messageLink = value
      }
  ...
  return { text: lines.join('\n'), title, version, updateAt, messageLink }
}
 ...
 */
/**
 * Read the one message the index points at, when the user has supplied a token.
 *
 * The index publishes a thread's metadata but not its text, and the version an
 * author states lives in the post body — "9月21日更新5.3" — which no amount of
 * parsing the title can recover. With a token this fetches exactly that message
 * and nothing else: one request, to one message id, on demand.
 * @param {string|null} link - the message link the index recorded.
 * @returns {Promise<string|null>} the message text, or null when it cannot be read.
 */
async function fetchUpdateMessage(link) {
  const token = String((db.cfg && db.cfg.discordToken) || '').trim()
  if (!token) return null
  const m = String(link || '').match(/channels\/(\d+)\/(\d+)\/(\d+)/)
  if (!m) return null
  const [, , channelId, messageId] = m
  try {
    const raw = await fetchText(`https://discord.com/api/v9/channels/${channelId}/messages/${messageId}`, 30000, {
      // Discord expects the token bare here, not behind a Bearer prefix.
      authorization: token,
      accept: 'application/json',
    })
    const msg = JSON.parse(raw)
    const parts = []
    if (msg && typeof msg.content === 'string' && msg.content) parts.push(msg.content)
    // Attachments carry no version, but their file names sometimes do, and they
    // cost nothing to include since the payload is already in hand.
    for (const a of (msg && msg.attachments) || []) {
      if (a && typeof a.filename === 'string') parts.push(a.filename)
    }
    return parts.length ? parts.join('\n') : null
  } catch (e) {
    log('discord message read failed:', e && e.message ? e.message : e)
    return null
  }
}
...
async function checkPrimary(entry, feedItems) {
  ...
  try {
    ...
    if (thread) {
      ...
      threadMessageLink = summary.messageLink
    ...
    // The post the index points at is fetched and read alongside the metadata when
    // a Discord token is configured. That is where authors state the version in
    // their own words, and the metadata does not carry it: "9月21日更新5.3" is in
    // the message body, not in the title. One request, to the one message the
    // index already named.
    const updateText = threadMessageLink ? await fetchUpdateMessage(threadMessageLink) : null
    const versionSource = updateText ? `${scope}\n${updateText}` : scope
    ...
    const info = threadVersion
      ? { value: threadVersion, kind: 'version' }
      : extractVersionInfo(versionSource)
    ...
    // An author renaming their thread is worth saying out loud: it is the same
    // card, and anyone who searched for the old name would think it had gone.
    const previousTitle = String(state.title || '')
    const renamed = !!(title && previousTitle && title !== previousTitle)
    ...
    state.messageRead = !!updateText
    state.renamedFrom = renamed ? previousTitle : null
    state.gates = gates
    state.newer = newer
    ...
    return {
      ...
      title: state.title,
      renamed,
      renamedFrom: renamed ? previousTitle : '',
      first,
      changed,
      newer,
      feed: feedNote,
      ...
      newUpdate,
      // Whether the post the index pointed at was actually read, so the panel can
      // say "the thread states no version" instead of leaving the user wondering.
      messageRead: !!updateText,
      gates,
      chars: scope.length,
    }
  } catch (e) {
