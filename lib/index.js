...
function threadSummary(payload) {
  const wanted =
    /^(title|name|thread_name|thread_title|last_message|last_message_at|updated_at|update_time|created_at|message_count|reply_count|first_message_excerpt|description)$/i
  const lines = []
  const seen = new Set()
  let title = ''
  let version = null
...
function extractVersion(text) {
  ...
  if (strongHits.length) return highest(strongHits)

  const weakHits = [...collect(weak, true), ...collect(weakTrailing, true)]
  if (weakHits.length) return highest(weakHits)
  ...
  return highest(bare.map((b) => b.replace(/^v/i, '')))
}
...
async function checkPrimary(entry, feedItems) {
  ...
  try {
    ...
    let text
    let title = ''
    // The index's own version for the thread, when it has one.
    ...
    // Only when it is empty does the text scan run.
    const version = threadVersion || extractVersion(scope)
