...
const DSH_HOME = process.env.DSH_HOME || 'C:\\Users\\213123543\\.dsh'
  ...
  switch (action) {
    ...
    case 'verifyIndex': {
      ...
      try {
        ...
        return { ok: true, configured: true, loggedIn: parsed.loggedIn === true }
      } catch (e) {
        const message = e && e.message ? e.message : String(e)
        return {
          ok: true,
    ...
    case 'openFolder': {
      ...
      // explorer.exe reports a non-zero exit even when it opens the window, so
      // the result is not inspected: the launch either happened or it did not.
      const run = await powershell(`Start-Process -FilePath explorer.exe -ArgumentList '${safe}'`, 15000)
      if (!run.ok && run.stderr && !/explorer/i.test(run.stderr)) throw new Error(run.stderr.split('\n')[0])
      return { ok: true, target }
    }
    case 'selfcheck':
