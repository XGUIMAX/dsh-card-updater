...
async function importPlain(cardId, fromPath) {
  ...
  // The MVU copy is named by hand, so its name cannot be derived from the
  // original's. Only a name that visibly contains the old one can follow it.
  const mvuPath = entry.mvu?.path
  const oldStem = renamed.from.replace(/\.json$/i, '')
  if (renamed.from && mvuPath && oldStem && mvuPath.includes(oldStem)) {
    const nextMvu = mvuPath.replace(oldStem, renamed.to.replace(/\.json$/i, ''))
    if (!exists(nextMvu)) {
      try {
