    ...
    if (path.resolve(nextMvu) !== path.resolve(mvuPath)) {
      if (exists(nextMvu)) {
        ...
        try {
          ...
          // The MVU copy has its own avatar, paired the same way. Without this it
          // falls back to the default picture the moment it is renamed.
          moveCardImages(wasStem, stem + ' ' + mark[2])
          out.chats = (out.chats || 0) + repointChats(`cards/${wasStem}.json`, `cards/${stem} ${mark[2]}.json`)
        } catch (e) {
