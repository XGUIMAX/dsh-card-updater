 ...
 * say the check is over while it is still going.
 */
const DEBUG_BUSY_MS = 5 * 60 * 1000
...
function cardWorkspaceState() {
  ...
  const cards = {}
  try {
    // chat id -> session id. Without the map a card still reports its debug time,
    // it just cannot be opened from here.
    ...
    const chats = path.join(TAVERN_DATA, 'chats')
    for (const chatId of fs.readdirSync(chats)) {
      const snapDir = path.join(chats, chatId, 'snapshots')
      let files
      try {
        files = fs.readdirSync(snapDir).filter((n) => n.endsWith('.json')).sort()
      } catch {
        continue
      }
      const last = files[files.length - 1]
      if (!last) continue
      const file = path.join(snapDir, last)
      let snap
      try {
        snap = JSON.parse(readText(file))
      } catch {
        continue
      }
      const name = snap && snap.cardPath ? path.basename(String(snap.cardPath)) : ''
      if (!name) continue
      let at = 0
      try {
      ...
      // The journal is appended on every write, so its mtime is the closest thing
      // to "this conversation is moving right now", and its contents say whether
      // the debug conversation ever wrote a card back.
      let movedAt = at
      let saved = false
      try {
        const jl = path.join(chats, chatId, 'journals')
        for (const f of fs.readdirSync(jl)) {
          const full = path.join(jl, f)
          const m = fs.statSync(full).mtimeMs
          if (m > movedAt) movedAt = m
          if (!saved) {
            try {
              if (readText(full).includes('"card.save"')) saved = true
            } catch {
              /* unreadable journal: read as having no save */
            }
          }
        }
      } catch {
        /* no journal yet: the snapshot time stands in */
      }
      const row = cards[name] || (cards[name] = { debugAt: null, sessionId: null, movedAt: 0, saved: false })
      const mode = String((snap && snap.mode) || '')
      if (mode === 'story' || mode === 'script') {
        if (!row.sessionId) row.sessionId = sessions.get(chatId) || null
      } else {
        if (at && (!row.debugAt || at > Date.parse(row.debugAt))) row.debugAt = new Date(at).toISOString()
        if (movedAt > row.movedAt) row.movedAt = movedAt
        if (saved) row.saved = true
      }
    }
    for (const row of Object.values(cards)) {
      row.busy = !!(row.movedAt && now - row.movedAt < DEBUG_BUSY_MS)
      // `card.save` means a debug conversation wrote changes back into the card
      // itself. Those changes are worth reading, and nothing on disk records
      // whether anyone has, so this reports the fact it can stand behind instead
      // of guessing at that one.
      row.changed = !!(row.saved && !row.busy)
      delete row.movedAt
      delete row.saved
    }
  } catch (e) {
